const FileSafe = artifacts.require("./FileSafe.sol");

require("chai").use(require("chai-as-promised")).should();

contract("FileSafe", ([deployer, uploader]) => {
	let fileSafe;

	before(async () => {
		fileSafe = await FileSafe.deployed();
	});

	describe("deployment", async () => {
		it("deploys successfully", async () => {
			const address = await fileSafe.address;
		...
		it("has a name", async () => {
			const name = await fileSafe.name();
			assert.equal(name, "FileSafe");
		});
	});
	...
	describe("file", async () => {
		...
		before(async () => {
			result = await fileSafe.uploadFile(fileHash, fileSize, fileType, fileName, fileDescription, { from: uploader });
			fileCount = await fileSafe.fileCount();
		});
		...
		it("upload file", async () => {
			...
			await fileSafe.uploadFile("", fileSize, fileType, fileName, fileDescription, { from: uploader }).should.be
				.rejected;

			// FAILURE: File must have size
			await fileSafe.uploadFile(fileHash, "", fileType, fileName, fileDescription, { from: uploader }).should.be
				.rejected;

			// FAILURE: File must have type
			await fileSafe.uploadFile(fileHash, fileSize, "", fileName, fileDescription, { from: uploader }).should.be
				.rejected;

			// FAILURE: File must have name
			await fileSafe.uploadFile(fileHash, fileSize, fileType, "", fileDescription, { from: uploader }).should.be
				.rejected;

			// FAILURE: File must have description
			await fileSafe.uploadFile(fileHash, fileSize, fileType, fileName, "", { from: uploader }).should.be.rejected;
		});

		//check from Struct
		it("lists file", async () => {
			const file = await fileSafe.files(fileCount);
